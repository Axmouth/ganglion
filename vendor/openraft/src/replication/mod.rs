    ...
    async fn send_log_entries(
        ...
        let (logs, sending_range) = {
            ...
            if start == end {
                ...
                // Retry without acknowledging data that was not read. A heartbeat
                // for this data request would report matching=None when prev is
                // absent, or acknowledge an unchanged prefix for a nonempty range.
                // The outer loop drains events between retries, including shutdown.
                if logs.is_empty() {
                    let sleep_duration = Duration::from_millis(10);
                    tracing::warn!(
                        "limited_get_log_entries({}, {}) returned empty; \
                         retrying without advancing replication progress. \
                    ...
                    return Ok(Some(Data::Logs(log_ids)));
                } else {
